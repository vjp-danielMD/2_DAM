...
class Pregunta1 : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val adapter = ArrayAdapter.createFromResource(this,R.array.hardware,android.R.layout.simple_spinner_item)
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
        spinner.adapter = adapter

        btnSiguiente.setOnClickListener {
            val respuestaSeleccionada = spinner.selectedItem.toString()
            val puntuacion = calcularPuntuacion(respuestaSeleccionada, "Placa base")

            // Recuperar puntuación acumulada
            ...
            val sinContestar =
                intent.getIntExtra("sinContestar", 0) + if (puntuacion == 0f) 1 else 0
